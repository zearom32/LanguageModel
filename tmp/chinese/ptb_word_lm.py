...
class PTBInput(object):
    ...
    def __init__(self, config, data, name=None):
        ...
        self.targets, self.input_data = reader.ptb_producer(
...
class PTBModel(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        ii = input_.input_data
        with tf.device("/cpu:0"):
            embedding = tf.get_variable(
                "embedding", [vocab_size, size], dtype=data_type())
            inputs = tf.nn.embedding_lookup(embedding, ii)
       ...
       # targets = tf.Print(targets,[ii, targets], "target")
...
def main(_):
    ...
    if FLAGS.inference:
        raw_data = reader.ch_inference_data(FLAGS.test)
